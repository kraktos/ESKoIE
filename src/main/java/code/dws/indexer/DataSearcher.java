...
public class DataSearcher
{
    ...
    public static Logger logger = Logger.getLogger(DataSearcher.class.getName());
     ...
     */
    public static BooleanQuery frameQuery(String userQuery, String userQuery2, String field1, String field2)
    {
        ...
        subQuery00.add(new TermQuery(new Term(field1, userQuery.toLowerCase())), BooleanClause.Occur.MUST);
        subQuery00.add(new TermQuery(new Term(field2, userQuery2.toLowerCase())), BooleanClause.Occur.MUST);

        subQuery01.add(new TermQuery(new Term(field1, userQuery.toLowerCase())), BooleanClause.Occur.MUST);
        subQuery01.add(new WildcardQuery(new Term(field2, userQuery2.toLowerCase() + "*")), BooleanClause.Occur.MUST);

        subQuery10.add(new WildcardQuery(new Term(field1, userQuery.toLowerCase() + "*")), BooleanClause.Occur.MUST);
        subQuery10.add(new TermQuery(new Term(field2, userQuery2.toLowerCase())), BooleanClause.Occur.MUST);

        subQuery11.add(new WildcardQuery(new Term(field1, userQuery.toLowerCase() + "*")), BooleanClause.Occur.MUST);
        subQuery11.add(new WildcardQuery(new Term(field2, userQuery2.toLowerCase() + "*")), BooleanClause.Occur.MUST);
     ...
     * @throws Exception
     */
    public static List<String> doSearch(String subjQuery, String objQuery) throws IOException
    {
        Query subQuery = null;
        ...
        try {
            ...
            reader = DirectoryReader.open(FSDirectory.open(new File(INDEX_DIR)));
            ...
            subQuery = frameQuery(subjQuery, objQuery, "oieSubField", "oieObjField");
            // subQuery = new TermQuery(new Term("oieSubField", userQuery.toLowerCase()));
     ...
     * @throws IOException
     */
    public static List<String> iterateResult(IndexSearcher searcher, TopDocs hits) throws IOException
    {
        String oieSub;
        String oieRel;
        String oieObj;
